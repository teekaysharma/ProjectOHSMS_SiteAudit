...
export function createApp({ dataDir, staticDir, jwtSecret } = {}) {
  const DATA_DIR = dataDir || path.join(__dirname, 'server-data');
  ...
  if (!fs.existsSync(STATE_FILE)) fs.writeFileSync(STATE_FILE, JSON.stringify({
  ...
  const distDir = staticDir !== undefined ? staticDir : path.join(__dirname, 'dist');
