...
function isValidStatePayload(state) {
  ...
  if (!state.masterConfig || !state.inspectionData) return false;
  if (typeof state.masterConfig !== 'object' || typeof state.inspectionData !== 'object') return false;
  if (!state.masterConfig.management || typeof state.masterConfig.management !== 'object') return false;
  if (!state.masterConfig.site || typeof state.masterConfig.site !== 'object') return false;
  if (!state.inspectionData.projects || typeof state.inspectionData.projects !== 'object') return false;
  if (typeof state.inspectionData.currentProject !== 'string') return false;

  // simple guardrail to avoid oversized payload abuse
  const payloadSize = Buffer.byteLength(JSON.stringify(state), 'utf8');
  return payloadSize <= 1024 * 1024; // 1MB
}
